...
void  setCmd(String s) {  if (s=="open") actionOpen(); else if (s=="close") actionClose(); else if (s == "reset") actionReset(); }
char* getTOE() { snprintf(strtmp, sizeof(strtmp), "%lu", flux.interruptCounter); return strtmp;}
void  setTOE(String s) { actionSetTotal( (uint64_t) s.toFloat() ); }
char* getMFREE() { snprintf(strtmp, sizeof(strtmp), "%lu", (unsigned long)ESP.getFreeHeap()); return strtmp;}
char* getHU() { snprintf(strtmp, sizeof(strtmp), "%.1f", humidityDHT); return strtmp;}
...
char* getTP() {snprintf(strtmp, sizeof(strtmp), "%.1f", temperatureDHT); return strtmp;}
...
String sentHtmlEau(){
  ...
  if (frame.server.method() == HTTP_POST || frame.server.method() == HTTP_GET) {
    for (uint8_t i=0; i<frame.server.args(); i++) { // Scan if Post or Get contain %%key%%
      if (cmd=='d') DBXMF("sentHtmlEau(Post or Get)  Arg->[%s]:[%s]\n\r", frame.server.argName(i), frame.server.arg(i) );
      if (frame.server.arg(i).isEmpty()) // Received a Get like "Cmd=" wihtout Arg will return "Ouvert" As text
        return callbackGetWwm(i); // Return immediate value
      else
        callbackSetWwm(i);
    }
    // Save if changed
